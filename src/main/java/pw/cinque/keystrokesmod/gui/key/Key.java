...
public class Key {
    ...
    private void drawSpaceBar(double keyHeight, double keyWidth, boolean pressed) {
        drawColoredRect(keyWidth * 0.25, keyHeight / 2.0 - 1.0, keyWidth * 0.75, keyHeight / 2.0 + 1.0, pressed);
    }

    private String getMouseText(boolean left) {
        int cps = (left ? keystrokesMod.getLeftClickCounter() : keystrokesMod.getRightClickCounter()).getCps();
        if (cps == 0) {
            return left ? "LMB" : "RMB";
        } else {
            return cps + " CPS";
    ...
    public Key(final KeyBinding keyBinding) {
        this.keyBinding = keyBinding;
    }

    @java.lang.SuppressWarnings("all")
    void setParent(final KeyHolder parent) {
        this.parent = parent;
    }

    @java.lang.SuppressWarnings("all")
    void setKeystrokesMod(final KeystrokesMod keystrokesMod) {
